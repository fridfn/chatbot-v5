...
if(localStorage.getItem("theme") === "basic") {
  ...
  console.log("ini adalah tema default")
...
footerEmail.innerHTML = `Kirimkan kritik & saran.`
...
function botStart(data) {
  ...
  var sendBtn = document.querySelector('.btn-bot');
  sendBtn.disabled = true;
...
function hujanLinear() {
   ...
   while(i < amount) {
    ...
    let duration = Math.random() * 34;
    
    drop.style.width = 2 + size+'px';
    drop.style.left = posX + 'px';
    drop.style.animationDelay = delay+'s';
    drop.style.animationDuration = 35 + duration + 's';
